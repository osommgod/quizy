...
/* ──────────  HELPERS  ────────── */
function fmtK($n){ return ($n>=1000)?number_format($n/1000,2).'K':number_format($n,2);}
function fmtNum($n){ return number_format($n,2,'.',','); }   // e.g. 1,523.00
...
/* ──────────  APP CONFIG  ────────── */
$withdraw_enabled=false; $min_withdraw_pts=0; $credit_value=1000;
...
while($c=$resCfg->fetch_assoc()){
  if($c['config_key']=='withdrawal_enabled') $withdraw_enabled=strtolower($c['config_value'])==='true';
  if($c['config_key']=='min_withdraw_amount')$min_withdraw_pts=(float)$c['config_value'];
  if($c['config_key']=='credit_value')       $credit_value=max(1,(int)$c['config_value']);
}
$resCfg->close();

if(!$withdraw_enabled){
   echo json_encode(["status"=>"error","message"=>"Withdrawals are currently disabled."]); $conn->close(); exit;
}
if($pointsRequest<$min_withdraw_pts){
   echo json_encode(["status"=>"error","message"=>"Minimum withdrawal is $min_withdraw_pts points"]); $conn->close(); exit;
}
...
$stmt->bind_param("s",$deviceid); $stmt->execute();
$user=$stmt->get_result()->fetch_assoc(); $stmt->close();
if(!$user){ echo json_encode(["status"=>"error","message"=>"User not found"]); $conn->close(); exit; }
...
if($pointsRequest>$totalPoints){
   echo json_encode(["status"=>"error","message"=>"Insufficient points"]); $conn->close(); exit;
}

/* ───── 1️⃣ deduct bonus points ───── */
$remainPts=$pointsRequest;
if($bonusPts >= $remainPts){ $bonusPts-=$remainPts; $remainPts=0;}
else{ $remainPts-=$bonusPts; $bonusPts=0; }

/* ───── 2️⃣ deduct credits ───── */
$remainCr=(int)round($remainPts*$credit_value);
if($credits >= $remainCr){ $credits-=$remainCr; $remainCr=0;}
else{ $remainCr-=$credits; $credits=0;}

if($remainCr>0){
   if($quiz >= $remainCr){ $quiz-=$remainCr; $remainCr=0;}
   else{ $remainCr-=$quiz; $quiz=0;}
}
if($remainCr>0){ $contest-=$remainCr; }
...
$ins=$conn->prepare("INSERT INTO withdraw
        ...
        VALUES(?,?,?,?,?,?,?,?,'Pending')");
$ins->bind_param("ssssssdd",$deviceid,$txn_id,$redeem_id,$redeem_method,$sim_circle,
                 $dateTime,$pointsRequest,$newUserPoints);
$success=$ins->execute(); $ins->close();
