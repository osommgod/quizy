...
function logHeadersHybrid($deviceid,$ip){
    ...
    foreach(getallheaders() as $k=>$v){ $entry.="$k: $v\n"; }
    $entry.="------------------------------\n";
...
function rotateLogs(){
    ...
    $today=date('Y-m-d');
    if(file_exists($flag)&&trim(file_get_contents($flag))===$today) return;
    ...
    foreach(scandir($baseDir) as $folder){
        ...
        if(!preg_match('/^\d{4}-\d{2}-\d{2}$/',$folder)) continue;
        $folderTime=strtotime($folder);
        if(!$folderTime||($now-$folderTime)<($zipAfterDays*86400)) continue;
        ...
        if($zip->open($zipFile,ZipArchive::CREATE|ZipArchive::OVERWRITE)){
            ...
            foreach($ri as $fi){ $fi->isDir()?rmdir($fi):unlink($fi); }
            rmdir($folderPath);
        }
    }
    ...
    foreach(scandir($zipDir) as $z){
        if(!str_ends_with($z,'.zip')) continue;
        $zipPath=$zipDir.$z;
        if((time()-filemtime($zipPath))>($deleteZipsAfter*86400)) unlink($zipPath);
    }
...
/* ─── Validate API/Device ─── */
if($api_key!==$valid_api_key){ http_response_code(401); jsonResponse("error","Invalid API key"); }
if(empty($deviceid)){ http_response_code(400); jsonResponse("error","Missing device ID"); }

/* ─── Connect DB ─── */
$conn=new mysqli($host,$user,$password,$dbname);
if($conn->connect_error){ http_response_code(500); jsonResponse("error","DB connection failed"); }

/* ─── Blocked IP ─── */
$cb=$conn->prepare("SELECT 1 FROM blocked_ips WHERE ip_address=?");
$cb->bind_param("s",$user_ip); $cb->execute(); $cb->store_result();
if($cb->num_rows>0){
    $log=$conn->prepare("INSERT INTO failed_attempts(ip_address,reason) VALUES(?, 'Blocked IP tried to register')");
    $log->bind_param("s",$user_ip); $log->execute(); $log->close();
    http_response_code(403); jsonResponse("error","Access denied. Your IP is blocked.");
}
$cb->close();

/* ─── Already registered? ─── */
$chk=$conn->prepare("SELECT 1 FROM user_data WHERE user_deviceid=?");
$chk->bind_param("s",$deviceid); $chk->execute(); $chk->store_result();
if($chk->num_rows>0){ $chk->close(); $conn->close(); jsonResponse("exists","Device already registered."); }
$chk->close();
...
while($r=$res->fetch_assoc()){
   if($r['config_key']=='signup_point')       $signup_point=(float)$r['config_value'];
   if($r['config_key']=='signup_bonuspoints') $signup_bonus=(float)$r['config_value'];
}
$res->close();
...
function uniqueUsername($conn,$retry=5){
   while($retry--){
     ...
     $c->bind_param("s",$u); $c->execute(); $c->store_result();
     if($c->num_rows==0){ $c->close(); return $u; }
     $c->close();
   }
   return null;
}
...
for($try=0;$try<3;$try++){
   ...
   if(!$uname) jsonResponse("error","Username generation failed.");
   ...
   try{
       ...
       $ntf->bind_param("ss",$deviceid,$msg); $ntf->execute(); $ntf->close();
